	...
	$query = '';
	$output = [];
	$query .= "SELECT * FROM product INNER JOIN brand ON brand.brandID = product.brandID INNER JOIN category ON category.categoryID = product.categoryID INNER JOIN user ON user.userID = product.productEnterBy ";
	...
	foreach($result as $row)
	{
		...
		$sub_array[] = '<button type="button" id="'.$row["productID"].'" class ="btn btn-danger btn-xs approveProduct" productName="'.$row["productName"].'" data-toggle="modal" data-target="#approveProductModal">Approve</button>';
		$data[] = $sub_array;
	}
	$draw = '';
	if(isset($_POST['draw']))
