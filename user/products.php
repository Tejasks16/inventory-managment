	...
	<div id="approveProductModal" class="modal fade">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header bg-red">
					<button type="button" style="color: #fff;" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title text-white"><span class="glyphicon glyphicon-trash"></span> Approve</h4>
				</div>
				<div class="modal-body">
				...
				<div class="modal-footer">
			        <button type="button" class="btn btn-dark" data-dismiss="modal">Cancel</button>
			        <input type="submit" class="btn btn-danger" id="approveProductBtn" value="Approve Product">
			     </div>
			</div>
		</div>
	</div>
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('click', '.editProduct', function(){
				...
				let checkboxElems = '';
				if(productEditStatus== 'pending'){
					 checkboxElems = `<label id="labelStatusProduct">
										<input type="checkbox" id="checkboxStatusProduct" checked> Pending
									  </label>`;
				}
				else{
					 checkboxElems = `<label id="labelStatusProduct">
										<input type="checkbox" id="checkboxStatusProduct" > Approved 
									  </label>`;
				}
				$('#productModal .modal-body div').remove('.checkbox');
			...
			$(document).on('click', '#checkboxStatusProduct', function(){
				...
				let str2 = str.trim();
				if(str2=='Pending'){
					$("#labelStatusProduct").html(`<input type="checkbox" id="checkboxStatusProduct" > Approved`);
				}
				else{
					$("#labelStatusProduct").html(`<input type="checkbox" id="checkboxStatusProduct" checked > Pending`);
				}
			});
			...
			$(document).on('click', '.approveProduct', function(){
				addProductToken = "<?php echo hash('sha256', 'asdfgh')  ?>";
				deleteProductToken = "<?php echo hash('sha256', 'approve_product_token')  ?>";
				updateProductToken = '<?php echo hash('sha256', 'abcdef')  ?>';
				$('#productBtnModal').removeClass('btn-warning updateProductSubmit');
				$('#productBtnModal').removeClass('btn-pink text-white addProductSubmit');
				$('#approveProductBtn').removeAttr('update_product_id');
				$('#approveProductBtn').attr('delete_product_id', $(this).attr('id'));
				$('#approveProductBtn').attr('delete_product_name', $(this).attr('productName'));
				$('#approveProductBtn').addClass('approveProductSubmit');
				$('#approveProductModal .modal-body').html(`<h5 class="text-black"> Are You Sure Want to Approve <strong>"`+ $(this).attr('productName') +`"</strong> Product ?</h5>`)
			});
			$(document).on('click','.approveProductSubmit', function(event){
				event.preventDefault();
				$('#approveProductBtn').attr('disabled', 'disabled');
				approveProductSubmit = true;
				const approveProductFormData = [
												{name : 'deleteProductToken', value : deleteProductToken},
												{name : 'productID', value : $(this).attr('delete_product_id')},
												{name : 'productName', value : $(this).attr('delete_product_name')}
											  ];
				$.ajax({
					...
					success:function(data){
						...
						$('#approveProductBtn').attr('disabled', false);
						productListData.ajax.reload();
					}
				})
			});
